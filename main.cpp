...
double scalar_product_parallel(sycl::queue& q, const std::vector<double>& a, const std::vector<double>& b, bool is_gpu)
{
    size_t n = a.size();
    double result = 0.0;
...
size_t roundup(size_t x, size_t multiple) {
    return ((x + multiple - 1) / multiple) * multiple;
}

std::vector<double> compute_jacobi_preconditioner(const std::vector<size_t>& row_ptr, const std::vector<size_t>& col_idx, const std::vector<double>& val, size_t n, sycl::queue& q)
{
    std::vector<double> M_inv(n, 1.0);
	
	sycl::buffer<double, 1> val_buf(val.data(), sycl::range<1>(val.size()));
	sycl::buffer<size_t, 1> col_ind_buf(col_idx.data(), sycl::range<1>(col_idx.size()));
	sycl::buffer<size_t, 1> row_ptr_buf(row_ptr.data(), sycl::range<1>(row_ptr.size()));
	
	{
		sycl::buffer<double, 1> M_buf(M_inv.data(), sycl::range<1>(n));
		...
		q.submit([&](sycl::handler& h) {
			...
			sycl::accessor M_access(M_buf, h, sycl::write_only, sycl::no_init);
			h.parallel_for<class Obus>(sycl::range<1>(n), [=](sycl::id<1> i) {
...
void CG_SYCL_full(std::vector<double>& K_val, std::vector<size_t>& K_col_ind, std::vector<size_t>& K_row_ptr, std::vector<double>& x, std::vector<double>& b, std::string& device)
{
    ...
    std::vector<double> M_inv = compute_jacobi_preconditioner(K_row_ptr, K_col_ind, K_val, n, q);
    ...
    q.submit([&](sycl::handler &h) {
        ...
        auto red_sum = sycl::reduction(old_rr_buf, h, sycl::plus<double>());

        h.parallel_for(sycl::range<1>(n), red_sum, [=](sycl::id<1> i, auto &sum) {
                sum += r_access[i] * z_access[i];
            }
        );
    }).wait();
    ...
    do {
        ...
        q.submit([&](sycl::handler &h) {
            ...
            auto red_sum = sycl::reduction(pAp_buf, h, sycl::plus<double>());

            h.parallel_for(sycl::range<1>(n), red_sum, [=](sycl::id<1> i, auto &sum) {
                    sum += p_access[i] * Ap_access[i];
                }
            );
        }).wait();
        ...
        q.submit([&](sycl::handler &h) {
            ...
            auto red_sum = sycl::reduction(new_rr_buf, h, sycl::plus<double>());
        ...
        q.submit([&](sycl::handler& h) {
            auto r_acc = new_rr_buf.get_access<sycl::access::mode::read>(h);
            h.copy(r_acc, &new_rr);
        }).wait_and_throw();

        // std::cout << "new_rr / b_norm = " << new_rr / b_norm << "   iter = " << iteration << std::endl;
        ...
        q.submit([&](sycl::handler& h) {
            sycl::accessor old_rr_access(old_rr_buf, h, sycl::write_only, sycl::no_init);
            sycl::accessor new_rr_access(new_rr_buf, h, sycl::read_only);
...
void CG_SYCL(std::vector<double>& K_val, std::vector<size_t>& K_col_ind, std::vector<size_t>& K_row_ptr, std::vector<double>& x, std::vector<double>& b, std::string& device)
{
    ...
    do {
        ...
        double old_rr = scalar_product_parallel(q, r, z, is_gpu);
        ...
        q.submit([&](sycl::handler& h) {
            auto acc = z_buf.get_access<sycl::access::mode::read>(h);
            h.copy(acc, z.data());
        });
        q.wait_and_throw();

        new_rr = scalar_product_parallel(q, r, z, is_gpu);
        ...
        q.submit([&](sycl::handler& h) {
            ...
            h.parallel_for(sycl::range<1>(n), [=](sycl::id<1> i) {
                p_access[i] = z_access[i] + beta * p_access[i];
            });
        }).wait();
...
int main(void){
	...
	CG_SYCL_full(K_val, K_col_ind, K_row_ptr, X3, b, CPU);
	timespec_get(&ts2, TIME_UTC);
	sec = (double(ts2.tv_sec) + double(ts2.tv_nsec) / 1000000000) - (double(ts1.tv_sec) + double(ts1.tv_nsec) / 1000000000);
	std::cout << "CPU full: " << sec << std::endl << std::endl;
	...
	timespec_get(&ts1, TIME_UTC);
	std::vector<double> X4(b.size());
	CG_SYCL_full(K_val, K_col_ind, K_row_ptr, X4, b, GPU);
	timespec_get(&ts2, TIME_UTC);
	sec = (double(ts2.tv_sec) + double(ts2.tv_nsec) / 1000000000) - (double(ts1.tv_sec) + double(ts1.tv_nsec) / 1000000000);
	std::cout << "GPU full: " << sec << std::endl << std::endl;
